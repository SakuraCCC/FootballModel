...
def run(competition: str, match_id: str, *, data_mode: str | None = None, allow_provider: bool = False, offline: bool = False, dry_run: bool = False) -> dict:
    ...
    if dry_run:
        return {"status": "not_executed", "reason": "dry_run", "data_mode": resolved_mode}
    ...
    try:
        ...
        return {"match_id": match.id, "competition": competition, "snapshot_ids": snapshots, "prediction_id": prediction.id, "report_id": report.report_id, "poster_id": Path(poster.file_path).stem, "archive_id": archive.id, "data_mode": resolved_mode, "data_completeness": "offline_incomplete" if resolved_mode == "offline" else "persisted", "model_status": prediction.status, "llm_status": report.status, "poster_file_status": Path(poster.file_path).exists(), "statistics_snapshot_id": stats.snapshot_id if stats else None, "provider_request_count": 0 if resolved_mode == "offline" else None, "provider_quota_remaining": None, "missing_fields": ["latest_injuries", "official_lineups", "live_statistics"] if resolved_mode == "offline" else []}
    finally:
        session.close()
